...
async function main() {
  ...
  console.log(`Working directory: ${config.defaultWorkDir || process.cwd()}`);
  ...
  // Check SDK stderr buffer (from llm-provider.ts code)
  console.log('\nFor actual CLI errors, check the stderr buffer in llm-provider.ts code');
}
